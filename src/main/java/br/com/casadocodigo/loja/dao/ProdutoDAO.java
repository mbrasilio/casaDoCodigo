...
public class ProdutoDAO {
	...
	public List<Produto> listaPorData(Calendar dataLancamento) {
		if (dataLancamento == null) {
			return manager.createQuery("select distinct(p) from Produto p order by p.dataLancamento", Produto.class)
					.getResultList();
		}
		TypedQuery<Produto> query = manager
				.createQuery("select distinct(p) from Produto p where p.dataLancamento >= :dataLancamento "
						+ "order by p.dataLancamento", Produto.class)
